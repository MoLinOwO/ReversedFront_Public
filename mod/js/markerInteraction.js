...
import { drawConnectionLines, clearConnectionLines } from './markerConnections.js';

let markerData = {};

async function ensureMarkerDataLoaded() {
    ...
    async function bindMarkers() {
        ...
        document.querySelectorAll('[class^="PortalMap_marker"], [class^="PortalMap_markerCapital"]').forEach(marker => {
            ...
            if (swords && swords.tagName === 'IMG') {
                ...
                // 只保留 transform，移除 backgroundColor、borderRadius、filter、boxShadow、background、border
            ...
            marker.onmouseenter = function(e) {
                ...
                if (data && data.airport) drawConnectionLines(marker, markerData, 'airport');
                if (data && data.port) drawConnectionLines(marker, markerData, 'port');
            };
